...
export default function PostDetail() {
  ...
  const handleAddComment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        setNewComment('');
        loadPostData(); // Recargar comentarios
      }
    } catch (error) {
  ...
  const handleDeletePost = async () => {
    if (!confirm('¿Estás seguro de eliminar este post? Todos los comentarios también se eliminarán.')) return;
  ...
  const canEdit = currentUser && currentUser.id === post.author_id;
